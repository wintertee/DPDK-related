...
```
#rm -rf /usr/local/etc/openvswitch/conf.db #需要删除吗不确定
...
ovs-vsctl --no-wait set Open_vSwitch . other_config:dpdk-lcore-mask=0x8 # 在cpu3运行
ovs-vsctl --no-wait set Open_vSwitch . other_config:pmd-cpu-mask=0x8 # 在cpu3运行
